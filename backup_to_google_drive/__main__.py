...
class SetLogging:
    ...
    FILE_FORMATTER = '{time} | {level:<8} | {process:<7} | ' \
...
class ConnGoogleDrive(ConfigMixin):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            raise Exception('No Google OAuth2 token found. '
    ...
    def upload_file(self, filepath: str) -> str:
        ...
        file_metadata = {
            ...
            ] if self.google_drive_backup_folder else None
        }
    ...
    def clean_backup_folder(self, actual_files: List[str]) -> None:
        ...
        """
        if not self.google_drive_backup_folder:
            log.error('Can\'t clean backup folder due to non specified '
                      'Google Drive backup folder id')
            return

        log.info(f'Deleting old backups files from backup folder '
                 f'in Google Drive...')
        files_to_del = [
            f for f in self._get_files_in_folder(
                self.google_drive_backup_folder
        ...
        for iteration in range(iterations):
            ...
            for i in range(
                    iteration * 100,
                    range_to if range_to < len(files_to_del) else
                    len(files_to_del)
            ):
                batch.add(self._drive_service.files().delete(
                    fileId=files_to_del[i]
                ))
        ...
        for n in range(0, 5):
            try:
                ...
                if not re.search(r'HttpError (500|503)', str(e)):
                    ...
                    raise e
                if n == 4:
                    error_text = 'Failed to complete request to ' \
                                 'Google Drive API in 5 attempts due ' \
                                 'to 500 or 503 error'
                    log.error(error_text)
                    raise Exception(error_text)
                log.debug(f'HTTP error: {str(e)[11:14]}. '
                          f'Trying again. Attempt {n + 1}')
...
class ZipMaker(ConfigMixin):
    ...
    def _prepare_folder_for_backup(self) -> Iterable[PreparedFile]:
        ...
        for address, dirs, files in os.walk(self.folder):
            ...
            for file in files:
                filepath = os.path.join(root_path, file) if \
                    address == absolute_root_path else \
                    os.path.join(
                        root_path,
                        os.path.relpath(
                            address,
                            start=absolute_root_path
                        ),
                        file)
    ...
    def run(self) -> str:
        ...
        """
        temp_back_folder = self.TEMP_BACKUP_FOLDER if \
            os.path.exists(self.TEMP_BACKUP_FOLDER) else \
            os.path.abspath(os.path.dirname(__file__))
        zipfile_name = f'{self.archive_name}_' \
...
def main():
    ...
    folders = list(set(args.folders)) if args.folders else \
        ConfigMixin().config.get('folders_for_backup')
    if not folders:
