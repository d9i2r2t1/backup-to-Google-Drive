...
GOOGLE_DRIVE_TOKEN_FILEPATH = os.path.join(os.path.dirname(__file__), GOOGLE_DRIVE_TOKEN_FILENAME)
...
class SetLogging:
    ...
    FILE_FORMATTER = '{time} | {level:<8} | {process:<7} | {name}:{function}:{line} – {message}'
    ...
    def __init__(self,
        ...
        :param log_to_file_kwargs: Дополнительные аргументы Loguru для настройки логирования в файл
        """
    ...
    def level(self, log_level: str, ) -> None:
        """
        Настрой логирование.
        ...
        if self.log_to_output:
            log.add(sys.stdout, level=log_level, format=self.STREAM_FORMATTER, enqueue=True)
        if self.log_to_file:
    ...
    def _validate_log_level(self, log_level: str) -> None:
        ...
        if log_level not in self.LOG_LEVELS:
            raise ValueError(f'Invalid log level: "{log_level}". It must be one of the list: {self.LOG_LEVELS}')
...
class ConfigMixin:
    ...
    def read_yaml_config(filepath: str) -> Dict[str, Any]:
        ...
        if not os.path.basename(filepath).endswith(('yml', 'yaml',)):
            raise TypeError('Invalid config file type. Only ".yml" or ".yaml" supported.')
        try:
...
class ConnGoogleDrive(ConfigMixin):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            raise Exception('No Google OAuth2 token found. Fetch it using --fetch-token argument.')
        ...
        self._drive_service = discovery.build('drive', 'v3', credentials=credentials, cache_discovery=False)
        self.google_drive_backup_folder = self.config.get('google_drive_backup_folder_id')
        if not self.google_drive_backup_folder:
            log.warning('No Google Drive backup folder ID specified. '
                        'The folders will be backed up to the Google Drive root folder')
    ...
    def fetch_token() -> None:
        ...
        print('Fetching Google Drive OAuth2 token.\n'
        ...
        token = oauth.fetch_token(token_url=token_uri, code=authorization_code, client_secret=client_secret)
        token.update({
            'token_uri': token_uri,
            'client_id': client_id,
    ...
    def upload_file(self, filepath: str) -> str:
        ...
        file_metadata = {
            'name': filename,
            'parents': [self.google_drive_backup_folder] if self.google_drive_backup_folder else None
        }
        media = MediaFileUpload(filepath, mimetype=ctype, resumable=True)
        file = self._exponential_backoff(
            self._drive_service.files().create(body=file_metadata, media_body=media, fields='id')
        )
        ...
        while True:
            response = self._exponential_backoff(
                self._drive_service.files().list(
                    q=query, pageSize=1000, spaces='drive', orderBy='folder', pageToken=page_token
                )
            )
    ...
    def clean_backup_folder(self, actual_files: List[str]) -> None:
        ...
        """
        if not self.google_drive_backup_folder:
            log.error('Can\'t clean backup folder due to non specified Google Drive backup folder id')
            return

        log.info(f'Deleting old backups files from backup folder in Google Drive...')
        files_to_del = [f for f in self._get_files_in_folder(self.google_drive_backup_folder) if f not in actual_files]
        iterations = math.ceil(len(files_to_del) / 100)  # Количество batch-запросов, за которые можно удалить файлы

        def callback(request_id, response, exception):  # noqa
            """Коллбек для запросов внутри batch-запроса."""
            if exception:
                log.warning(f'Request #{(iteration * 100) + int(request_id)} failed: {exception}')

        for iteration in range(iterations):
            time.sleep(2)  # Задержка для попадания в ограничение API по кол-ву запросов
            batch = self._drive_service.new_batch_http_request(callback=callback)
            range_to = (iteration * 100) + 100
            for i in range(iteration * 100, range_to if range_to < len(files_to_del) else len(files_to_del)):
        ...
        for n in range(0, 5):
            try:
                ...
                if not re.search(r'HttpError (500|503)', str(e)):
                    log.error(f'Failed to complete request to Google Drive API due to error: {str(e)}')
                    raise e
                if n == 4:
                    error_text = 'Failed to complete request to Google Drive API in 5 attempts due to 500 or 503 error'
...
class ZipMaker(ConfigMixin):
    ...
    def _prepare_folder_for_backup(self) -> Iterable[PreparedFile]:
        ...
        for address, dirs, files in os.walk(self.folder):
            ...
            for file in files:
                filepath = os.path.join(root_path, file) if address == absolute_root_path else \
                    os.path.join(root_path, os.path.relpath(address, start=absolute_root_path), file)
                prepared_files.append(self.PreparedFile(
                    absolute_filepath=os.path.join(address, file),
                    filepath=filepath)
                )
    ...
    def run(self) -> str:
        ...
        """
        temp_back_folder = self.TEMP_BACKUP_FOLDER if os.path.exists(self.TEMP_BACKUP_FOLDER) else \
            os.path.abspath(os.path.dirname(__file__))
        zipfile_name = f'{self.archive_name}_{datetime.now().strftime("%Y%m%d_%H-%M-%S")}.zip'
        ...
        with zipfile.ZipFile(zipfile_path, 'w') as z:
            for file in self.files:
                if file.absolute_filepath == zipfile_path:
                    continue
                try:
                    z.write(filename=file.absolute_filepath, arcname=file.filepath, compress_type=zipfile.ZIP_DEFLATED)
                except FileNotFoundError:
...
def make_backup(folder: str) -> str:
    ...
    """
    SetLogging(log_filename=LOG_FILENAME, rotation='20 MB', backtrace=True, diagnose=True).level(LOG_LEVEL)
...
def main():
    ...
    parser.add_argument('-f', '--folders', nargs='+', type=str, help='paths to folders to backup to Google Drive')
    parser.add_argument('--no-backup-clean', action='store_true', help='don\'t delete old backups in Google Drive')
    parser.add_argument('--fetch-token', action='store_true', help='only fetch Google Drive Oauth2 token')
    ...
    folders = list(set(args.folders)) if args.folders else ConfigMixin().config.get('folders_for_backup')
    if not folders:
